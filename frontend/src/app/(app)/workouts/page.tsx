              ...
              <div className="mt-4 space-y-3 border-t pt-4">
                <div className="grid grid-cols-2 gap-4 text-sm">
                  {session.paceMinPerKm && (
                    <div>
                      <span className="text-muted-foreground">Pace:</span> {session.paceMinPerKm.toFixed(2)} min/km
                    </div>
                  )}
                  {session.steps && (
                    <div>
                      <span className="text-muted-foreground">Steps:</span> {session.steps.toLocaleString()}
                    </div>
                  )}
                  {session.intensityLevel && (
                    <div>
                      <span className="text-muted-foreground">Intensity:</span> {session.intensityLevel}/5
                    </div>
                  )}
                  {session.preEnergy && (
                    <div>
                      <span className="text-muted-foreground">Pre-workout Energy:</span> {session.preEnergy}/5
                    </div>
                  )}
                </div>
                {session.postMood && (
...
function WorkoutModal({
  ...
  const [activeTab, setActiveTab] = React.useState<'basic' | 'metrics' | 'notes'>('basic');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await onSave({
      ...
      paceMinPerKm: formData.paceMinPerKm ? Number(formData.paceMinPerKm) : undefined,
      steps: formData.steps ? Number(formData.steps) : undefined,
      avgHeartRate: formData.avgHeartRate ? Number(formData.avgHeartRate) : undefined,
  ...
  React.useEffect(() => {
    if (formData.distanceKm && formData.durationMin) {
  ...
  }, [formData.distanceKm, formData.durationMin]);
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          ...
          {activeTab === 'notes' && (
            <>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-medium">Pre-workout Energy (1-5)</label>
                  <Input
                    type="number"
                    min="1"
                    max="5"
